...
\begin{proof}
    The level sets can be written as $f^{-1}(I)$ and $f^{-1}(C)$ respectively, where $I = (\alpha,\infty)$ and $C = [\alpha,\infty)$ are open and closed set in $\R$. Then by continuity of $f$, we have that $f^{-1}(I)$ is open and $f^{-1}(C)$ is closed in $\R^n$. The convexity of the level sets follows from Theorem \ref{thm:convex_level_sets}. 
\end{proof}
