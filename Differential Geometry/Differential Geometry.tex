...
\begin{definition}
Suppose $\M$ is a topological space. $\M$ is a \highlight{topological manifold of dimension $n$} or a \highlight{topological n-manifold} if it has the following properties:
\begin{enumerate}[(a)]
    \item $\M$ is a \highlight{Hausdorff space}.
    \item $\M$ is a \highlight{second-countable}.
    \item $\M$ is \highlight{locally Euclidean of dimension $n$}.
\end{enumerate}
...
\begin{prop}[\highlight{Equivalent Characterizations of Smoothness}]
...
\begin{prop}[\highlight{Smoothness is Local}]
...
\begin{prop}[\highlight{Gluing Lemma for Smooth Maps}]
...
\begin{prop}[\highlight{Properties of Differentials}]
...
\begin{prop}[\highlight{Tangent vectors act locally}]
Let $\M$ be a smooth manifold, $p \in \M$ and $v\in T_p\M$. If $f, g \in \Cinf(M)$ agree on some neighborhood of $p$, then $vf = vg$.
\end{prop}

\begin{prop}[\highlight{The Tangent Space to an Open Submanifold}]
...
\begin{prop}[\highlight{The Tangent Space to a Product Manifold}]
