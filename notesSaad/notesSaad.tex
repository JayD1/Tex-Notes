...
Assume $x_0$ is the initial guess such that $r_0=b-Ax_0$ such that $r_0\perp W.$ Now set $v_1=r_0/\|r_0\|_2$ and define $\mathcal{K}_{k,j}(A,W,r_0)\equiv\text{span}\{W,V_j\}$. At j-th step of the projection, we seek for the approximate solution $x_j\in x_0+\mathcal{K}_{k,j}(A,W,r_0)$ and $r_j=b-Ax_j\perp \mathcal{K}_{k,j}(A,W,r_0).$

\begin{lemma}
If $x_i\;and\;r_j$ satisfy the above conditions, then $r_j=c_jv_{j+1},$ for some scalar $c_j$. Thus $\mathcal{K}_{k,j}(A,W,r_0)=\text{span}\{W,r_0,r_1,\cdots,v_{j-1}\}$ and the residuals $r_i$'s are orthogonal to each other.
\end{lemma}
...
We know, $V^Tr_j=0\implies 0 = V^Tr_0-T_j\hat{\eta}_j\implies \hat{\eta}_j = \|r_0\|_2T^{-1}_je_1$. Now substituting this equation in the above, we get
\begin{align*}
r_j &= r_0-\|r_0\|_2V_jT_j(T^{-1}_je_1)-\|r_0\|_2\sigma_{j+1}v_{j+1}e^T_j(T^{-1}_je_1) \\
\implies r_j &= (-\sigma_{j+1}e^T_jT^{-1}_je_1)v_{j+1} \\
